...
![please clone](https://raw.githubusercontent.com/whsgzcy/i2c_by_time-/master/images/general.png)
...
![please clone](https://raw.githubusercontent.com/whsgzcy/i2c_by_time-/master/images/driver_device_register_i2c_bus.png)
...
其中的i2c_bus_type对象，其表示i2c总线，定义了设备和驱动的匹配规则还有匹配成功后的行为
...
```
device_register向总线注册设备

device_register首先会将设备添加到总线的设备链表中，然后遍历总线的驱动链表，判断设备和驱动是否匹配，如果匹配就去掉驱动的probe函数
...
其中bus_add_device函数会将设备添加到总线设备链表中
...
发现它调用了总线的match函数，这里的总线在注册i2c设备的时候已经被设置为i2c总线了
...
如果匹配成功会调用driver_probe_device，driver_probe_device
...
_driver_attach函数，此函数会判断设备和驱动是否匹配，如果匹配就调用驱动的probe函数
...
对于三星平台在drivers/i2c/busses/i2c_s3c2410.c文件中构建并注册了i2c适配器，这是三星平台的i2c控制器的驱动
...
s3c24xx_i2c_algorithm中的s3c24xx_i2c_xfer 就是通过操作寄存器来通过i2c控制传输数据的

**3.4、i2c数据传输**

i2c数据传输是通过i2c适配器完成的

在i2c驱动中，使用i2c_transfer来传输i2c数据，此函数肯定是通过i2c适配器的算法进行操作的
...
**基本全部参考 作者 程序员JT 博客，所展示代码均由本项目中截取，感谢作者** 

**本项目地址**
https://gitee.com/whsgzcy/LR-RK3288-kernel/tree/lr_rk3288_kernel_i
